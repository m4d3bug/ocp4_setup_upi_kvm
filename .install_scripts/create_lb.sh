...
virt-customize -a "${VM_DIR}/${CLUSTER_NAME}-lb.qcow2" \
    --uninstall cloud-init --ssh-inject root:file:${SSH_PUB_KEY_FILE} --selinux-relabel --install haproxy --install bind-utils --install dnsmasq\
    --hostname lb.${CLUSTER_NAME}.${BASE_DOM} \
    ...
    --run-command "systemctl daemon-reload" --run-command "systemctl enable tmpws.service" --run-command "systemctl enable dnsmasq.service" || \
...
ssh -i sshkey "root@$LBIP" \
    "cat /etc/hosts.${CLUSTER_NAME} >> /etc/hosts" || err "Adding hosts on lb.${CLUSTER_NAME}.${BASE_DOM} failed"; ok
ssh -i sshkey "root@$LBIP" \
